...
TEST "Select" = (select (db) "Name,Number FROM Phonebook") = db
...
TEST "Insert with multiple specified columns" =
  let db' = insert big_db "Stuffs (Stuff 2, Stuff 5, Stuff 4, Stuff 3) VALUES
            (1, 3.4, true, false)" in
...
let drop_db = [{title="Customers";cols=[{name="CustomerID";vals=[VInt(1);VInt(2);
              ...
              vals=[VString("Alfreds Futterkiste");
              VString("Ana Trujillo Emparedados y helados");
              VString("Antonio Moreno Taqueria");VString("Around the Horn");
              VString("Berglunds snabbkop")];typ=TString};
              {name="ContactName";vals=[VString("Maria Anders");
              VString("Ana Trujillo");VString("Antonio Moreno");
              VString("Thomas Hardy");VString("Christina Berglund")];
...
TEST "Select with wrong column name" =
  try let _ = select db "Phonebook from Phonebook" in false
  with _ -> true

TEST "Select with wrong table name" =
  try let _ = select db "Name from Number" in false
  with _ -> true
